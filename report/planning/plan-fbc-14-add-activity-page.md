...
-   [x] `Textarea.tsx` component exists in `presentation/components/ui/`
-   [x] Component follows same pattern as `Input.tsx` (label, helper text, error handling)
-   [x] Component uses `getFormFieldIds` from `shared/a11y/utils.ts` for accessibility IDs
-   [x] Component supports all standard textarea props (value, onChange, onBlur, placeholder, disabled, required)
-   [x] Component displays error messages with `role="alert"` and proper ARIA attributes
-   [x] Component uses SCSS variables from `styles/variables/*` for all styling
-   [x] Component is memoized with `React.memo`
-   [x] Component has TypeScript types for all props
-   [x] Component follows arrow function with export default pattern

**Definition of Done:**

-   [x] Component created and follows Input component pattern
-   [x] Component styled using SCSS variables (no hardcoded values)
-   [x] Component accessible (WCAG 2.1 AA): proper labels, ARIA attributes, error announcements
-   [x] Component tested (unit tests in `__tests__/presentation/components/ui/Textarea.test.tsx`)
-   [x] Lint/build passes
...
-   [x] `useAddActivity` hook exists in `presentation/hooks/useActivities.ts` (or separate file)
-   [x] Hook uses `useMutation` from React Query
-   [x] Hook calls `addActivity` usecase with `activityRepositorySupabase`
-   [x] Hook invalidates activities list queries on success (using `queryKeys.activities.list()`)
-   [x] Hook invalidates dashboard recent activities query on success (using `queryKeys.dashboard.recentActivities()`)
-   [x] Hook returns mutation object with `mutate`, `isPending`, `error`, `isSuccess` properties
-   [x] Hook properly types mutation input as `Omit<Activity, 'id'>`
-   [x] Hook properly types mutation result as `Activity`
-   [x] Hook properly types error as `ActivityError | Error`

**Definition of Done:**

-   [x] Hook created following React Query mutation patterns
-   [x] Hook invalidates all related queries on success
-   [x] Hook properly typed with TypeScript
-   [x] Hook tested (unit tests in `__tests__/presentation/hooks/useActivities.test.tsx`)
-   [x] Lint/build passes
...
-   [x] `AddActivityForm.tsx` component exists in `presentation/components/addActivity/`
-   [x] Component uses activity type select (CREATION, SALE, STOCK_CORRECTION, OTHER)
-   [x] Component shows/hides fields dynamically based on activity type:
    -   [x] CREATION: quantity (number, required), product (select, required), note (textarea, optional)
    -   [x] SALE: product (select, required), quantity (number, required, typically negative), amount (number, required), note (textarea, optional)
    -   [x] STOCK_CORRECTION: product (select, required), quantity (number, required, can be positive or negative), amount (number, required), note (textarea, optional)
    -   [x] OTHER: quantity (number, required), amount (number, required), note (textarea, optional), product (select, optional)
-   [x] Component includes date field (defaults to current date/time, ISO 8601 format)
-   [x] Component uses `useAddActivity` hook for mutation
-   [x] Component displays loading state during submission (disabled form, loading button)
-   [x] Component displays field-level validation errors accessibly
-   [x] Component displays general error messages accessibly (aria-live region)
-   [x] Component uses all UI components from design system (Input, Select, Textarea, Button)
-   [x] Component uses `getFormFieldIds` for accessibility IDs (via UI components)
-   [x] Component uses SCSS variables for all styling
-   [x] Component is memoized appropriately
-   [x] Component has proper TypeScript types

**Definition of Done:**

-   [x] Component created with dynamic field rendering
-   [x] Component handles all activity types correctly
-   [x] Component validates inputs and displays errors accessibly
-   [x] Component uses design system components
-   [x] Component styled using SCSS variables (no hardcoded values)
-   [x] Component accessible (WCAG 2.1 AA): proper labels, ARIA attributes, error announcements, live regions
-   [x] Component tested (unit tests in `__tests__/presentation/components/addActivity/AddActivityForm.test.tsx`)
-   [x] Lint/build passes
-   [x] Component documented with JSDoc
...
-   [x] Page exists at `app/dashboard/activities/new/page.tsx`
-   [x] Page renders `AddActivityForm` component
-   [x] Page displays success message accessibly (aria-live region) after successful submission
-   [x] Page redirects to `/dashboard/activities` after successful submission (with delay for feedback)
-   [x] Page handles navigation back to activities list (cancel button or link)
-   [x] Page uses proper semantic HTML structure
-   [x] Page uses SCSS variables for all styling
-   [x] Page has proper page title and heading
-   [x] Page is accessible (WCAG 2.1 AA): proper heading hierarchy, skip links, focus management

**Definition of Done:**

-   [x] Page created with form and navigation
-   [x] Page handles success feedback and redirect
-   [x] Page styled using SCSS variables (no hardcoded values)
-   [x] Page accessible (WCAG 2.1 AA): proper structure, headings, live regions
-   [x] Page tested (unit tests in `__tests__/app/dashboard/activities/new/page.test.tsx` - if needed, but page tests are optional per rules)
-   [x] Lint/build passes
-   [x] Page documented with JSDoc
...
-   [x] Accessibility ID constants added to `shared/a11y/ids.ts`:
    -   [x] `formSuccess: "form-success"`
    -   [x] `formError: "form-error"`
-   [x] Constants exported and typed properly
-   [x] Constants used in AddActivityForm component
-   [x] Constants follow existing naming patterns

**Definition of Done:**

-   [x] Constants added to `shared/a11y/ids.ts`
-   [x] Constants used in form components
-   [x] Constants follow existing patterns
-   [x] Lint/build passes
